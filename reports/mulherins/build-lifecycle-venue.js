'use strict';
// Per-venue Inquiry -> BEO Lifecycle report.
// Usage: node build-lifecycle-venue.js <venueId> "<Venue Name>" "<Subtitle>" <outSlug>
// Reads deep_all.json (all venues), filters to venueId, renders <outSlug>-lifecycle.html
const fs = require('fs'), path = require('path');
const DIR = __dirname;
const [VID, VNAME, VSUB, SLUG] = [process.argv[2], process.argv[3], process.argv[4], process.argv[5]];
if (!VID || !VNAME || !SLUG) { console.error('args: <venueId> "<Venue Name>" "<Subtitle>" <outSlug>'); process.exit(1); }
const ALL = JSON.parse(fs.readFileSync(path.join(DIR, 'deep_all.json'), 'utf8'));
const L = ALL.filter(r => String(r.venue_id) === String(VID)).map(r => ({ ...r, status: r.lead_status, guest_count: r.guests }));
if (!L.length) { console.error('no rows for venue', VID); process.exit(1); }
...
const brand = sub => `<div class="brand"><div class="n">${esc(VNAME)}<span>${esc(VSUB || 'Method Co')}</span></div><div class="doc">${sub}</div></div>`;
...
let p1 = `<div class="page">${brand('Daily Inquiry → BEO Report')}
  ...
  <div class="kpis">
    ${kpi('Median Response', (respMedian == null ? '—' : respMedian + (respMedian === 1 ? ' day' : ' days')), 'mut', 'to first disposition')}
  ...
  <div class="foot">Same-day disposition on ${sameDay} of ${disposed.length} decided inquiries (${pct(sameDay, disposed.length)}). "Response" is measured in calendar days; conversions are stamped by date in Tripleseat, so same-day = 0.</div>
</div>`;
...
const topOwnerShare = byOwner.length && evRows.length ? byOwner[0][1] / evRows.length : 0;
let p2 = `<div class="page">${brand('Inquiry → BEO Lifecycle · Breakdowns')}
  ...
  <div style="display:flex;gap:24px">
    ...
    <table class="tbl"><thead><tr><th class="l">Owner</th><th>Bookings</th><th>Share</th></tr></thead><tbody>${ownRows || '<tr><td class="l">—</td><td class="r">0</td><td class="r">—</td></tr>'}</tbody></table></div>
    <div style="flex:1"><div style="font-size:10px;color:#5a6b86;margin-bottom:3px">Created by</div>
    <table class="tbl"><thead><tr><th class="l">Creator</th><th>Bookings</th><th>Share</th></tr></thead><tbody>${creRows || '<tr><td class="l">—</td><td class="r">0</td><td class="r">—</td></tr>'}</tbody></table></div>
  </div>
  <div class="note">Leads arrive unowned in Tripleseat; every built <b>booking</b> is owned. Top owner here is <b>${esc(byOwner[0]?.[0] || '—')}</b> (${pct(byOwner[0]?.[1] || 0, evRows.length)} of built bookings), created by <b>${esc(byCreator[0]?.[0] || '—')}</b>. Booking ownership is tracked; the gap is only at the raw lead stage, where the web form doesn't assign an owner until a booking is built.</div>
...
fs.writeFileSync(path.join(DIR, SLUG + '-lifecycle.html'), html);
console.log(`${VNAME}: total ${total} | booked ${booked} | pipeline ${inPipe} | lost ${cnt('LOST')} | value ${money(confirmedVal)} -> ${SLUG}-lifecycle.html`);
