...
TA=CFG.get('toast_anth') or {}
if TA: TA['base']=TA['net']-TA['svc']-TA['adm']
TK=CFG.get('toast_kamp') or {}
...
rows=[
 ...
 row("Rental &amp; fees",cat(A,'RENTAL'),(t('fees')+'*') if TA else '',cat(K,'RENTAL'),t('rental',TK),0),
 row("Standalone rooftop / Rotunda events",0,'',cat(K,'UNITEMISED'),'',cat(RO,'UNITEMISED')),
 row('Base revenue (Event Actual)',an['base'],t('base'),km['base'],t('base',TK),ro['base'],True),
 row('Service charge',an['svc'],t('svc'),km['svc'],(t('svcgrat',TK)+'†') if TK else '',ro['svc']),
 row('Admin fee',an['adm'],t('adm'),km['adm'],t('adm',TK),ro['adm']),
 row('Net revenue',an['net'],t('net'),km['net'],t('net',TK),ro['net'],True),
]
...
<div class="cards">
<div class="card"><div class="label">Anthology · net</div><div class="value">${f0(an['net'])}</div><div class="sub">{("Toast $"+f0(TA['net'])+f" · {an['net']-TA['net']:+,.0f} ({100*(an['net']-TA['net'])/TA['net']:+.1f}%)") if TA else "Toast report not yet provided"}</div></div>
...
<div class="card"><div class="label">Kamper's · event net</div><div class="value">${f0(km['net'])}</div><div class="sub">{("Toast A/R $"+f0(TK['net'])+f" · {km['net']-TK['net']:+,.0f} ({100*(km['net']-TK['net'])/TK['net']:+.1f}%)") if TK else "Toast report not yet provided"}</div></div>
