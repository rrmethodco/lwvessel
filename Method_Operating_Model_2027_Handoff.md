...
**File:** `Method_Operating_Model_2027_34.xlsx` (26 tabs, 0 formula errors, recalculated via LibreOffice)
...
(v34: `LW Item Forecast` fully ungrouped — every category lists every individual Toast item, no "All other" rows: NA Beverage 82 · Food 304 · Liquor 17 · Bottled Beer 21 · Wine 19 · Retail 45 · Uncategorized 19 = 507 item rows, ~540 rows total. Actual sales stored at exact cents so the grand total ties to LW Rev Recon POS net $133,092 to the dollar.)

(v32: new **`LW Item Forecast`** tab (after LW Rev Recon H1) — item-level monthly forecast grid. Each Toast sales category (NA Beverage, Food, Liquor, Bottled Beer, Wine, Retail, Uncategorized) lists its individual items (top sellers + "All other" remainder rows): actual units & POS net sales Jan–Jun by month, avg net price, **editable Rec +$ (blue, col B)** seeded from deck slide 9 (Latte/Drip +$0.50, Cold Brew +$0.45, Cappuccino +$0.50, Americano/Draft Latte +$0.25, Matcha +$0.50, Egg & Cheese sandwiches +$1.00, smoothies +$1.00), new price, **editable Jul–Dec forecast units (blue, cols S–X, seeded = Apr–Jun avg)**, forecast sales = units × new price. Subtotals per category, grand total ties to LW Rev Recon POS net ($133,092, ±$2 rounding), and a comparison row vs `LW 2026 Modified` Total Income (GL basis) for Jul–Dec. Analysis/forecast tool — does NOT drive the plan tabs; scenario outputs unchanged.)

(v31: **`LW Rev Recon H1` — unreconciled revenue classified as Other Income.** New "RESTATED MONTHLY INCOME" block (rows 14–25): Jan–Jun income restated on the POS category basis (Food/NA/Liquor/Beer/Wine/Retail/Uncategorized at Toast net) with the non-POS residual plugged to an **Other Income** line (`=reported − SUM(POS categories)`), so every month ties to the reported P&L total to the dollar (check row = $0 ×6). H1 Other Income plug = +$1,589 (Jan +$2,261 carries almost all of it; Feb–Jun slightly negative = check-level comps). Presentation-only — the LW 2026 P&L accountant replica and all scenario outputs are untouched. Findings block reframed as "treatment adopted": non-POS revenue → Other Operating Income.)
...
(v24: Vessel 2027 rebuilt on the per-event unit-economics card — $4,650 revenue/event (F&B $3,720 + 20% svc + 5% admin), COGS $1,810 (food 55.6%/bev 25%), loaded labor $806 (incl manager $55 + bonus $112/event), controllable 12.4%, uncontrollable 2.5% → NOP $1,157/event × 83 events. All 12 combined months profitable.)

(v23 cleanup levers: food COGS → 40% of food sales · G&A-unc → 4.5% of revenue · discounts/comps → 2.5% of gross · retail COGS → 42%. LW turns slightly profitable; ALL 12 combined months ≥ breakeven; Aug–Dec immediate impact +$103,820.)

(v21 — controllable target ratios in modified/plan: Vessel OpEx 3% + G&A-C 4% of revenue; LW OpEx 12.5% + Mktg 3.5% + G&A-C 8.5%. Aug–Dec immediate impact: LW +$53.3K, Vessel +$37.2K, combined +$90.5K.)

(v16 — synced to the Plan-of-Action deck: pricing = deck slide 9 list prices on TTM units → gross $10,179, net $9,161 @90%; retail programming raised to $42K/yr rev @50% COGS = ~+$24K GP; LW shift = 6:00a–4:00p (10h), doors 6:30a.)
